 ...
 * 明示的に大きく確保している。
 */
async function ocrImage(blob, { signal, bookMode } = {}) {
  const imageBase64 = await blobToBase64(blob);
  // Almagest(書籍・記事の取り込み)から呼ばれた時だけ、書籍向けの指示を足す(2026年9月追加)。
  // 画像を見ているOCRの段階でこそ確実に判断できる「本文ではないもの」の除外と改行の整理、
  // 見出しへの「■ 」付けだけに留める。誤字の修正・言い換えはここではさせない(変更前との比較が
  // 残らず気づけないため)。それは添削欄(「🪄 自動添削」)で確認しながら行う分担にしている。
  const bookInstruction = bookMode
    ? 'これは書籍・雑誌・記事のページです。以下の点だけは例外として扱ってください。' +
      '(1) ルビ(振り仮名)は書き写さず、親文字だけを書くこと。' +
      '(2) ページ番号(ノンブル)、欄外の柱(書名・章名などの繰り返し表示)、図版番号だけの断片は書き写さないこと。' +
      '(3) 紙面の都合による行末の改行はつなげ、1つの段落は1行にまとめること。段落の切れ目だけは空行で区切ること。' +
      '(4) 見出し・小見出し(本文より文字が大きい・太い・前後に余白がある等、見た目から見出しと判断できる行)は、' +
      '独立した1行にし、行頭に「■ 」を付けること。本文の行には付けないこと。' +
      'これら以外は、誤字に見える文字や不自然な言い回しも含めて、書かれている通りに書き写し、直したり言い換えたりしないこと。'
    : '';
